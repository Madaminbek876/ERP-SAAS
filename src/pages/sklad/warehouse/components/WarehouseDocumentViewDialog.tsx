...
type WarehouseDocumentBadge = {
  label: string
  tone?: BadgeTone
}

type WarehouseDocumentAction = {
  label: string
  onClick: () => void
  disabled?: boolean
...
function actionClass(variant: WarehouseDocumentAction["variant"]) {
  if (variant === "outline") {
...
export default function WarehouseDocumentViewDialog({
  open,
  onOpenChange,
  title,
  badges = [],
  ...
  title: string
  badges?: WarehouseDocumentBadge[]
  summary?: ReactNode[]
  actions?: WarehouseDocumentAction[]
  leftTitle: string
  fields: WarehouseDocumentField[]
  tabs: WarehouseDocumentTab[]
  loading?: boolean
  loadingLabel?: string
  error?: string
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[calc(100vw-24px)] !max-w-[1320px] max-h-[92vh] overflow-y-auto rounded-[32px] border border-slate-200 bg-transparent p-0 shadow-none"
      >
